...
import { dataUrlToFile, getErrorMessage, nextFileToDataUrl, readJson, shorten, toCandidate, type AnchorReceipt, type Candidate, type Notice, type Verification, type WorkflowState } from "./components/shared";
...
function SpecimenRoute({ file, previewUrl, embeddingSize, quality, phash, isDragging, searching, searchId, candidates, onChange, onDrop, onDragOver, onDragLeave, onOpenSources, onRerun }: {
  file: File | null; previewUrl: string; embeddingSize: number | null; quality: number | null; phash: string; isDragging: boolean; searching: boolean; searchId: string; candidates: Candidate[];
  ...
  return <section className="route on" id="r-specimen"><div className="wrap">
    <div className="pg-k">01 // SPECIMEN — FACE INTAKE</div><div className="pg-row"><h2 className="pg-t">Measure the face.</h2><button className="btn" disabled={!file || searching} onClick={onRerun}><RefreshCw size={13} />RE-RUN THE SCAN</button></div>
    ...
    <div className="scan-grid"><div><SpecimenCard file={file} previewUrl={previewUrl} embeddingSize={embeddingSize} quality={quality} phash={phash} isDragging={isDragging} searching={searching} searchId={searchId} onChange={onChange} onDrop={onDrop} onDragOver={onDragOver} onDragLeave={onDragLeave} /></div><div className="panel route-sidecard"><div className="p-head"><span>CASE INTAKE — LIVE</span><span className="st">{searching ? "RUNNING" : candidates.length ? "READY" : "WAITING"}</span></div><div className="p-body"><h3>{candidates.length ? "Face scan complete." : "Start with one face image."}</h3><p>{candidates.length ? "The provider returned sources. Continue to the discovery board to choose the evidence you want to seal." : "Nothing is searched until a specimen is selected. Raw image buffers stay in memory for this run."}</p><button className="btn btn-p btn-w btn-lg" disabled={!candidates.length || searching} onClick={onOpenSources}>VIEW THE SOURCES <ArrowRight size={14} /></button></div></div></div>
  </div></section>;
}
...
export default function Page(): ReactElement {
  ...
  async function runSearch(nextFile: File): Promise<void> {
    ...
    try {
      ...
      if (runId !== activeRunRef.current) return;
      setSearchId(nextSearchId); setProvider(resultProvider); setCandidates(nextCandidates); setSelectedId(nextCandidates[0]?.id ?? ""); setWorkflow(nextCandidates.length ? "sources" : "idle"); setNotice(nextCandidates.length ? { kind: "success", text: `${nextCandidates.length} public source${nextCandidates.length === 1 ? "" : "s"} returned with search ID ${shorten(nextSearchId, 10, 6)}.` } : { kind: "info", text: "The provider returned no public source for this face. That is a valid result; nothing was invented." });
    } catch (error) {
      if (runId !== activeRunRef.current) return;
      setWorkflow("idle"); setNotice({ kind: "error", text: `Analysis stopped: ${getErrorMessage(error)}` });
    }
  }
  ...
  function resetCase(): void { setFile(null); setPreviewUrl(""); setPhash(""); setEmbeddingSize(null); setQuality(null); setCandidates([]); setSelectedId(""); setSearchId(""); setProvider(""); setWorkflow("idle"); setNotice(null); setAnchor(null); setVerification(null); openRoute("home"); }
  ...
  return <main className="sybil-app">
    ...
    {route === "specimen" && <SpecimenRoute file={file} previewUrl={previewUrl} embeddingSize={embeddingSize} quality={quality} phash={phash} isDragging={isDragging} searching={workflow === "analyzing"} searchId={searchId} candidates={candidates} onChange={handleFileChange} onDrop={handleDrop} onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onOpenSources={() => openRoute("sources")} onRerun={() => { if (file) void runSearch(file); }} />}
